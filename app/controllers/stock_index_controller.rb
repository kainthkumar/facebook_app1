class StockIndexController < ApplicationController
  def index
    begin
      #get response from stock_server, using configurations into config/settings.yml and request query into url
      response = RestClient.get "#{Rails.configuration.stock_server_url}/#{Rails.configuration.stock_index_controller}?#{URI.escape(request.query_string)}"
